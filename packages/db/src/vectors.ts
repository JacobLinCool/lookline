/**
 * Cosine ranking in SQL without an extension. `product_vectors` keeps a unit-normalised copy of
 * every `products.style_vector` spread over 64 REAL columns (`v0` … `v63`; created by
...
/** Only `product_id` is declared; the 64 value columns are addressed by `cosineExpr`. */
export const productVectors = sqliteTable('product_vectors', {
...
/**
 * `cos(query, product_vectors row)` as a SQL expression in `[-1, 1]`. `table` is the (unaliased)
 * table name of the joined `product_vectors`. Returns `0` for a zero query vector so ORDER BY
 * still has a well-defined tie-break.
 */
export function cosineExpr(query: readonly number[], table = 'product_vectors'): SQL<number> {
...
export function productVectorRow(styleVector: readonly number[]): number[] {
  return unitVector(styleVector).map((x) => Number(x.toFixed(6)))
}

/**
 * One multi-row `INSERT OR REPLACE INTO product_vectors` with inlined literals (no parameters).
 * Seeds pass a few hundred rows per call; the statement stays under 100 KB.
 */
export function productVectorsInsertSql(
  rows: ReadonlyArray<{ id: number; styleVector: readonly number[] }>,
): SQL {
  if (rows.length === 0) throw new Error('productVectorsInsertSql: no rows')
  const cols = ['product_id', ...VECTOR_COLUMNS].map((c) => `"${c}"`).join(',')
  const values = rows
    .map((r) => `(${r.id},${productVectorRow(r.styleVector).map(literal).join(',')})`)
    .join(',')
  return sql.raw(`insert or replace into product_vectors (${cols}) values ${values}`)
}
