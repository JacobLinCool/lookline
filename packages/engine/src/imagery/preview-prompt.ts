/**
 * `buildPreviewImagePrompt`: the image-model prompt for a private preview. Person first
...
export function describeGarment(p: PreviewPromptInput['articles'][number]): string {
...
/**
 * The same outfit photograph, composed from attached images instead of catalog rows: the person
...
/**
 * The labels for a preview's reference images, in the order the provider receives them: one
 * `Garment N` per article with `hasImage`, in article order, then the person when attached.
 */
export function previewReferenceLabels(
  articles: PreviewPromptInput['articles'],
  hasReferencePhoto: boolean,
): readonly string[] {
 ...
 * then reads as part of the outfit. Name the gaps and keep them plain. Silent when roles are unknown.
 */
export function describeFillers(articles: PreviewPromptInput['articles']): string | null {
...
export function buildPreviewImagePrompt(input: PreviewPromptInput): string {
  const { preset, articles, ownerName, occasion, hasReferencePhoto } = input
  const labels = previewReferenceLabels(articles, hasReferencePhoto)
