/**
 * `renderCardPosterSvg`: a deterministic 900×1200 placeholder for cards and previews. The preset's ground
...
import { findPreviewArtPreset } from './preview-presets'
...
type Theme = PreviewArtPreset['theme']
...
function resolveTheme(slug: string, palette: readonly string[]): Theme & { name: string } {
  const preset = findPreviewArtPreset(slug)
...
function wrapTitle(title: string, maxChars: number, maxLines: number): string[] {
  ...
  return lines.length > 0 ? lines : ['Untitled card']
}
...
function composeShapes(
  articles: CardPosterInput['articles'],
  rng: Rng,
  ink: string,
 ...
 */
function layOutBands(input: CardPosterInput, rng: Rng, ink: string): Band[] {
...
export function renderCardPosterSvg(input: CardPosterInput): string {
  const rng = createRng(hashSeed(input.seed, 'card-poster'))
