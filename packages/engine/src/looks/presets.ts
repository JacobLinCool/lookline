/**
 * Editorial style presets for Look images (LLM prompt fragment + poster theme).
...
const PRESET_BY_SLUG: ReadonlyMap<string, StylePreset> = new Map(
  STYLE_PRESETS.map((p) => [p.slug, p]),
)

export function findStylePreset(slug: string): StylePreset | undefined {
